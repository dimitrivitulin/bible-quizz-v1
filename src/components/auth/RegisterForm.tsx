...
export default function RegisterForm() {
  ...
  return (
    <div className="w-full bg-parchment-card border border-gold-subtle rounded-2xl p-8">
      <h1 className="font-serif text-3xl text-sepia mb-1 text-center">Créer un compte</h1>
      <p className="text-sepia-subtle text-sm italic text-center mb-6">Commence ton parcours dans la Parole</p>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <div>
          <label htmlFor="displayName" className="block text-sm text-sepia-muted mb-1">Prénom</label>
          ...
          {errors.displayName && <p className="text-[#7A2232] text-xs mt-1">{errors.displayName.message}</p>}
        </div>
        ...
        <div>
          <label htmlFor="confirmPassword" className="block text-sm text-sepia-muted mb-1">Confirmer le mot de passe</label>
          <Input id="confirmPassword" type="password" autoComplete="new-password" {...register('confirmPassword')}
            className="bg-parchment border-gold-subtle text-sepia placeholder:text-sepia-subtle focus:border-[#A0762A] text-base"
            placeholder="••••••••" />
          {errors.confirmPassword && <p className="text-[#7A2232] text-xs mt-1">{errors.confirmPassword.message}</p>}
        </div>
